...
strikeFX <- function(data, geom = "point", point.size=3, point.alpha=1/3, color = "pitch_types", density1=list(), density2=list(), contour=FALSE, adjust=TRUE, layer = list(), limitz=c(-2.5, 2.5, 0, 5), ...){ 
  ...
  if (geom %in% c("bin", "hex", "tile")) { #special handling for (2D) density geometries
    ...
    if (geom %in% c("bin", "tile")) t2 <- t2 + stat_summary2d(aes(x=environment$x,y=environment$y,z=environment$z), ...) 
    if (geom %in% "hex") t2 <- t2 + stat_summary_hex(aes(x=environment$x,y=environment$y,z=environment$z), ...)
    if (contour) t2 <- t2 + stat_contour(aes(x=environment$x,y=environment$y,z=environment$z)) #passing binwidth here throws error
