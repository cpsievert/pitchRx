...
animateFX <- function(data, layer = list(), geom = "point", point.color = aes(color = pitch_types), point.alpha = aes(alpha = 0.5), point.size = 100-FX$y, breaks = c(0,5,10), interval = 0.01, sleep = 0.000000000001, ...){ 
  ...
  for (i in 1:dim(snapshots)[2]) {
    ...
    Sys.sleep(sleep)
    p <- ggplot() + xlim(-3.5, 3.5) + xlab("Horizontal Pitch Location") + ylim(0, 6) + ylab("Height from Ground") + scale_size(guide="none") + scale_alpha(guide="none") + theme(legend.position = c(0.25,0.05), legend.direction = "horizontal") + scale_color_brewer(palette="Set2")
    if (geom %in% "point") {
      #FX$sizes <- point.size
